-- Older offline sales may have a discounted total but no discount field in their queued payload.
-- Recover that discount only for managers/admins, and persist it as an audited discount.
...
declare
  ...
  effective_discount_kobo bigint := p_discount_kobo;
begin
  ...
  -- Lock and calculate first so an old queued discount can be reconstructed safely.
  for sale_item in select (value->>'product_id')::uuid as product_id, (value->>'quantity')::integer as quantity from jsonb_array_elements(p_items) loop
    ...
    select price_kobo into server_price_kobo from public.products
  ...
  if effective_discount_kobo = 0 and p_total_kobo < computed_total_kobo then
    effective_discount_kobo := computed_total_kobo - p_total_kobo;
  end if;
  ...
  if effective_discount_kobo > computed_total_kobo or computed_total_kobo - effective_discount_kobo <> p_total_kobo then
    raise exception 'Sale total does not match current catalogue prices (expected %, submitted %)', computed_total_kobo - effective_discount_kobo, p_total_kobo;
  end if;
  ...
  values (p_sale_id, current_store, auth.uid(), p_receipt_no, p_total_kobo, p_payment_method, p_sold_at, effective_discount_kobo, case when effective_discount_kobo > 0 then auth.uid() else null end);
