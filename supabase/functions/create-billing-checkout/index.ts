...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (planCode === 'enterprise')
      return json({ error: 'Kroniqos Plus requires a tailored commercial agreement.' }, 400)
    const { data: profile } = await admin.from('profiles').select('store_id,role').eq('id', user.id).single()
    if (!profile || profile.role !== 'admin')
      return json({ error: 'Only company administrators can manage billing.' }, 403)
    ...
    const { data: plan } = await admin
      ...
      .select('code,name,description,monthly_price_kobo,paystack_plan_code,active')
      .eq('code', planCode)
      .single()
    if (!store || !plan?.active || !plan.monthly_price_kobo)
      return json({ error: 'This plan is unavailable for online billing.' }, 400)
    let paystackPlanCode = plan.paystack_plan_code
    if (!paystackPlanCode) {
      const remotePlan = await paystack(secret, '/plan', {
        method: 'POST',
        body: JSON.stringify({
          name: `Naira POS ${plan.name}`,
      ...
      await admin
        .from('subscription_plans')
        .update({ paystack_plan_code: paystackPlanCode })
        .eq('code', plan.code)
    }
    ...
    const transaction = await paystack(secret, '/transaction/initialize', {
      method: 'POST',
      body: JSON.stringify({
        email: user.email,
        amount: String(plan.monthly_price_kobo),
        plan: paystackPlanCode,
        reference,
        callback_url: callbackUrl,
        metadata: { organization_id: store.organization_id, plan_code: plan.code },
      }),
    })
    return json({ authorizationUrl: transaction.authorization_url, reference })
  } catch (error) {
    return json({ error: error instanceof Error ? error.message : 'Could not start billing checkout.' }, 500)
  }
})
