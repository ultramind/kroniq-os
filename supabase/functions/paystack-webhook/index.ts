...
async function digest(algorithm: AlgorithmIdentifier, value: string) {
  return hex(await crypto.subtle.digest(algorithm, new TextEncoder().encode(value)))
}
...
Deno.serve(async (request) => {
  ...
  const { error: insertError } = await admin
    ...
    .insert({ event_key: eventKey, event_type: event.event, payload: event })
  ...
  try {
    ...
    if (event.event === 'subscription.create') {
      ...
      if (organizationId && subscriptionCode)
        await admin
          .from('organization_subscriptions')
          .update({
            provider: 'paystack',
            provider_subscription_id: subscriptionCode,
            provider_customer_id: readString(customer?.customer_code),
            status: 'active',
            past_due_at: null,
            grace_period_ends_at: null,
            updated_at: new Date().toISOString(),
    ...
    if (event.event === 'invoice.payment_failed') {
      ...
      const subscriptionCode =
        readString(subscription?.subscription_code) ?? readString(data.subscription_code)
      const query = admin
        .from('organization_subscriptions')
      ...
      if (failedOrganizationId)
        await admin
          ...
          .eq('organization_id', failedOrganizationId)
    }
    if (event.event === 'subscription.disable') {
      const subscriptionCode = readString(data.subscription_code)
      const { data: disabledSubscription } = await admin
        .from('organization_subscriptions')
        .select('organization_id')
        .eq('provider_subscription_id', subscriptionCode ?? '')
        .maybeSingle()
      ...
      if (disabledOrganizationId) {
        await admin
          ...
          .eq('organization_id', disabledOrganizationId)
