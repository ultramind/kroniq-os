...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: requester } = await adminClient
      ...
      .eq('user_id', user.id)
      .maybeSingle()
    if (requester?.platform_role !== 'owner')
      return json({ error: 'Only platform owners can add team members.' }, 403)
    ...
    if (!normalizedEmail || !password || password.length < 8 || !role || !roles.includes(role))
      return json({ error: 'Enter a valid email, temporary password, and role.' }, 400)
    ...
    if (createError || !memberId) {
      ...
      if (tenantProfile)
        return json(
          { error: 'This email belongs to a tenant staff account and cannot be added to the platform team.' },
          409,
        )
      const { error: updateError } = await adminClient.auth.admin.updateUserById(memberId, {
        password,
        ban_duration: 'none',
        email_confirm: true,
      })
    ...
    const { error: membershipError } = await adminClient
      .from('platform_admins')
      .insert({ user_id: memberId, platform_role: role })
    if (membershipError) {
    ...
    await adminClient.from('platform_audit_events').insert({
      actor_id: user.id,
      action: 'platform_team_member_added',
      after_data: { user_id: memberId, email: normalizedEmail, role },
    })
