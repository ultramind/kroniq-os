...
async function resolveSaleItems(items: CartItem[]) {
  ...
  const { data, error } = await supabase.from('products').select('id, sku').in('sku', items.map((item) => item.sku))
  ...
  if (missing) return { error: `Cannot sync ${missing.name}: it is not in the Supabase catalogue. Reload the catalogue, then retry.` }
  return { items: items.map((item) => ({ ...item, id: isUuid(item.id) ? item.id : idsBySku.get(item.sku)! })) }
}
...
export async function syncOutbox(): Promise<{ synced: number; error?: string }> {
  ...
  for (const event of events) {
    ...
    if (event.entity === 'shift_open' || event.entity === 'shift_close') {
      ...
      const { error } = await supabase.rpc(event.entity === 'shift_open' ? 'open_cash_shift' : 'close_cash_shift', event.entity === 'shift_open' ? { p_shift: payload } : { p_shift_id: payload.shiftId, p_closure: payload })
      ...
      await db.transaction('rw', db.shifts, db.outbox, async () => { await db.shifts.update(shiftId, { synced: true }); await db.outbox.delete(event.id!) })
      synced++
      continue
    }
    if (event.entity === 'sale_return') {
      const payload = event.payload as { saleId: string; operationId: string; activityId?: string; items: Array<{ productId?: string; saleItemId?: string; quantity: number }> }
      const returnItems = await Promise.all(payload.items.map(async (item) => {
      ...
      if (returnItems.some((item) => !item.product_id)) return { synced, error: 'Return sync: product details are unavailable on this device.' }
      const { error } = await supabase.rpc('return_sale_items', { p_sale_id: payload.saleId, p_operation_id: payload.operationId, p_items: returnItems })
      if (error) return { synced, error: `Return sync: ${error.message}` }
      await db.transaction('rw', db.sales, db.returnActivities, db.outbox, async () => { await db.sales.update(payload.saleId, { synced: true }); if (payload.activityId) await db.returnActivities.update(payload.activityId, { synced: true }); await db.outbox.delete(event.id!) })
      synced++
      continue
    }
    if (event.entity === 'stock_movement') {
      const movement = event.payload as StockMovement
      const { error } = await supabase.rpc('adjust_stock', { p_product_id: movement.productId, p_quantity_delta: movement.quantityDelta, p_reason: movement.reason, p_operation_id: movement.id })
      if (error) return { synced, error: `Stock adjustment sync: ${error.message}` }
      await db.transaction('rw', db.stockMovements, db.outbox, async () => { await db.stockMovements.update(movement.id, { synced: true }); await db.outbox.delete(event.id!) })
      synced++
      continue
    }
    if (event.entity === 'stock_delivery') {
      const payload = event.payload as { id: string; productId: string; supplierName: string; quantity: number; unitCost: number; receivedAt?: string }
      const { error } = await supabase.rpc('record_stock_delivery', { p_delivery: { id: payload.id, product_id: payload.productId, supplier_name: payload.supplierName, quantity: payload.quantity, unit_cost_kobo: Math.round(payload.unitCost * 100), received_at: payload.receivedAt } })
      if (error) return { synced, error: `Supplier delivery sync: ${error.message}` }
      await db.transaction('rw', db.stockDeliveries, db.outbox, async () => { await db.stockDeliveries.update(payload.id, { synced: true }); await db.outbox.delete(event.id!) })
      synced++; continue
    }
    ...
    if (resolved.error || !resolved.items) return { synced, error: resolved.error ?? 'Could not resolve the products for this sale.' }
    ...
    if (sale.paymentMethod === 'credit' && (sale.creditInitialPayment ?? 0) > 0) {
      const { error: creditError } = await supabase.rpc('record_credit_initial_payment', { p_sale_id: sale.id, p_initial_payment_kobo: Math.round((sale.creditInitialPayment ?? 0) * 100) })
...
export async function pullSales(): Promise<{ loaded: number; error?: string }> {
  ...
  await db.transaction('rw', db.sales, db.saleItems, async () => {
    for (const row of data ?? []) {
      ...
      // Never overwrite a locally-created sale until its own outbox event is acknowledged.
...
export async function pullDeliveries(): Promise<{ loaded: number; error?: string }> {
  ...
  const deliveries: StockDelivery[] = (data ?? []).map((row) => {
    ...
    return { id: row.id, productId: row.product_id, supplierName: supplier?.name ?? 'Unknown supplier', quantity: row.quantity, unitCost: row.unit_cost_kobo / 100, receivedAt: row.received_at, createdAt: row.created_at, synced: true }
  })
