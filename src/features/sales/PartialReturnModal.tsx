...
export function PartialReturnModal({ sale, open, saving, onClose, onSubmit }: Props) {
  ...
  const selected = useMemo(
    () => items.flatMap((item) => (quantities[item.id] ? [{ item, quantity: quantities[item.id] }] : [])),
    [items, quantities],
  )
  ...
  return (
    <Modal
      open={open}
      title={sale ? `Return items · ${sale.receiptNo}` : 'Return items'}
      okText={`Record return${returnTotal ? ` · ${formatNaira(returnTotal)}` : ''}`}
      okButtonProps={{ danger: true, disabled: !selected.length }}
    ...
    >
      ...
      <Table
        ...
        pagination={false}
        dataSource={items}
        locale={{ emptyText: 'Sale items are unavailable on this device.' }}
        columns={[
          {
            title: 'Item',
            dataIndex: 'productName',
            key: 'productName',
            render: (name: string, item: SaleItem) => (
              <div>
          ...
          { title: 'Sold', dataIndex: 'quantity', key: 'quantity' },
          {
            ...
            key: 'returned',
            render: (_: unknown, item: SaleItem) => item.returnedQuantity ?? 0,
          },
          {
            title: 'Return now',
            key: 'returnNow',
            render: (_: unknown, item: SaleItem) => {
              ...
              return (
                <InputNumber
                  ...
                  onChange={(value) =>
                    setQuantities((current) => ({ ...current, [item.id]: Number(value ?? 0) }))
                  }
                />
              )
            },
