...
export function ReceiptModal({ sale, items, onClose }: Props) {
  const [brand, setBrand] = useState<{ companyName: string; logoUrl: string }>({ companyName: '', logoUrl: '' })
  ...
  return <Modal open title="Sale complete" onCancel={onClose} footer={<Space><Button onClick={onClose}>Close</Button><Button type="primary" icon={<PrinterOutlined />} onClick={() => window.print()}>Print receipt</Button></Space>}>
    <section id="receipt" className="mx-auto max-w-sm text-sm">
      <div className="text-center">
        {brand.logoUrl ? <img src={brand.logoUrl} alt={`${businessName} logo`} className="mx-auto mb-2 h-12 max-w-40 object-contain" /> : null}
        <Title level={4} className="!mb-1">{businessName}</Title>
        <Text type="secondary">{settings.address || 'Sales receipt'}{settings.phone ? ` · ${settings.phone}` : ''}</Text>
      </div>
      <Divider />
      ...
      {sale.paymentMethod === 'credit' && <div className="mt-3 space-y-2 border-t border-slate-200 pt-3"><div className="flex justify-between"><Text type="secondary">Initial deposit</Text><Text>{formatNaira(initialPayment)}</Text></div><div className="flex justify-between text-base"><Text strong>Balance due</Text><Text strong>{formatNaira(creditBalance)}</Text></div>{sale.creditDueDate && <div className="flex justify-between"><Text type="secondary">Expected payment</Text><Text>{new Date(`${sale.creditDueDate}T12:00:00`).toLocaleDateString('en-NG', { dateStyle: 'medium' })}</Text></div>}</div>}
