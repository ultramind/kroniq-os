...
import { PlatformAnalyticsPage, PlatformAuditLogPage } from './PlatformObservability'
import { PlatformOperationsPage, PlatformStatusPage, PlatformSupportPage } from './PlatformSupportOps'
...
type PageProps = {
  ...
  stores: Store[]
  loading: boolean
  onStatusChange: (organization: Organization, status: OrganizationStatus) => Promise<void>
  updatingId?: string
  onRefresh: () => void
  plans: Plan[]
...
function OrganizationList({
  ...
  const subscriptionByOrganization = useMemo(
    () => new Map(subscriptions.map((subscription) => [subscription.organization_id, subscription])),
    [subscriptions],
  )
  ...
  const columns: TableColumnsType<Organization> = [
    {
      ...
      render: (_, organization) => (
        <button className="text-left" onClick={() => navigate(`/platform/organisations/${organization.id}`)}>
    ...
    },
    {
      ...
      responsive: ['sm'],
      render: (_, organization) => (
        <Tag>{subscriptionByOrganization.get(organization.id)?.plan_code ?? '—'}</Tag>
      ),
    },
    {
      title: 'Stores',
    ...
    },
    {
      ...
      responsive: ['lg'],
      render: (_, organization) => (
        <span className="capitalize">{subscriptionByOrganization.get(organization.id)?.status ?? '—'}</span>
      ),
    },
    {
      title: 'Status',
  ...
  return (
    <>
      ...
      <div className="mb-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Card>
          <Statistic title="Active" value={summary.active} valueStyle={{ color: '#15803d' }} />
        </Card>
        <Card>
          <Statistic title="Trials" value={summary.trial} valueStyle={{ color: '#2563eb' }} />
        </Card>
        <Card>
          <Statistic title="Stores" value={summary.stores} prefix={<TeamOutlined />} />
        </Card>
      </div>
...
function OrganizationDetail({
  ...
  const organizationStores = stores.filter((store) => store.organization_id === organizationId)
  const subscription = subscriptions.find((item) => item.organization_id === organizationId)
  ...
  useEffect(() => {
    ...
    const loadDetail = async () => {
      ...
      const [profileResult, metricResult, auditResult, organizationResult] = await Promise.all([
        storeIds.length
          ? client.from('profiles').select('id,store_id,full_name,role,created_at').in('store_id', storeIds)
      ...
      if (!metricResult.error) setMetrics((metricResult.data?.[0] ?? undefined) as Metrics | undefined)
      if (!metricResult.error) setMetricsError(undefined)
      else {
        setMetrics(undefined)
        setMetricsError(metricResult.error.message)
      }
      if (!auditResult.error) setActivity((auditResult.data ?? []) as PlatformAudit[])
      if (!organizationResult.error)
        setProfile((organizationResult.data?.[0] ?? undefined) as OrganizationProfile | undefined)
  ...
  return (
    <>
      ...
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Tag color={statusColor[organization.status]} className="capitalize">
              {organization.status}
            </Tag>
          </div>
          <Typography.Text type="secondary">
      ...
      <div className="mb-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Card>
          <Statistic title="Products" value={metrics?.product_count ?? 0} loading={detailLoading} />
        </Card>
        <Card>
          <Statistic title="Sales" value={metrics?.sale_count ?? 0} loading={detailLoading} />
        </Card>
        <Card>
          <Statistic
            title="Sales value"
        ...
        <Card>
          <Statistic title="Staff" value={metrics?.staff_count ?? 0} loading={detailLoading} />
        </Card>
      </div>
      {metricsError && (
        <Alert
          ...
          type="warning"
          showIcon
          message="Organisation metrics could not load"
          description="Run the latest platform metrics migration, then refresh this page."
        />
      )}
        ...
        <div className="space-y-6">
          ...
          >
            <Descriptions column={{ xs: 1, sm: 2 }} size="small">
              <Descriptions.Item label="Business email">{profile?.business_email ?? '—'}</Descriptions.Item>
              ...
              <Descriptions.Item label="Plan">
                <Select
                  ...
                  loading={updatingId === organization.id}
                  onChange={(planCode) => void onPlanChange(organization, planCode)}
                  options={plans
              ...
              renderItem={(store) => (
                <List.Item>
                  <List.Item.Meta
                    ...
                    title={
                      <span>
                        {store.name} {store.is_primary && <Tag className="ml-2">Primary</Tag>}
                      </span>
                    }
                  ...
                  <Select
                    ...
                    onChange={(status) => void onStoreStatusChange(store, status as 'active' | 'inactive')}
                    options={[
              ...
              renderItem={(member) => (
                ...
                >
                  <List.Item.Meta
                    ...
                    title={
                      <span>
                        {member.full_name} {member.role === 'admin' && <Tag className="ml-2">Owner</Tag>}
                      </span>
                    }
            ...
            renderItem={(event) => (
              <List.Item>
                <List.Item.Meta
                  ...
                  description={
                    <span>
                      {event.before_data?.status ?? event.before_data?.plan_code ?? '—'} →{' '}
                      {event.after_data?.status ?? event.after_data?.plan_code ?? '—'} ·{' '}
      ...
      >
        <Form form={form} layout="vertical" onFinish={(values) => void submitProfile(values)}>
          ...
          <Form.Item name="businessEmail" label="Business email" rules={[{ type: 'email' }]}>
            <Input />
          </Form.Item>
...
export function PlatformApp() {
  ...
  const load = useCallback(async () => {
    if (!supabase) return
    setLoading(true)
    const [organizationResult, subscriptionResult, storeResult, planResult] = await Promise.all([
      supabase.rpc('platform_organization_overview'),
      supabase
        .from('organization_subscriptions')
        .select('organization_id,plan_code,status,trial_ends_at,current_period_ends_at'),
    ...
    setSubscriptions(subscriptionResult.error ? [] : (subscriptionResult.data as Subscription[]))
  ...
  const updateProfile = useCallback(
    async (
      ...
      if (!supabase) return
      setUpdatingId(organization.id)
      const { error } = await supabase.rpc('update_platform_organization_profile', {
        p_organization_id: organization.id,
        p_name: values.name,
  ...
  const resendInvite = useCallback(
    async (organizationId: string, staffId: string) => {
      if (!supabase) return
      const { error } = await supabase.functions.invoke('platform-organization-operations', {
            ...
            : pathname.startsWith('/platform/status')
              ...
              : pathname.startsWith('/platform/operations')
                ...
                : pathname.startsWith('/platform/team')
                  ...
                  : 'dashboard'
  const can = (
    area: 'organizations' | 'plans' | 'analytics' | 'audit' | 'support' | 'status' | 'operations' | 'team',
  ) =>
    platformRole === 'owner' ||
    (platformRole === 'operator' &&
      ['organizations', 'plans', 'analytics', 'audit', 'status', 'operations'].includes(area)) ||
    (platformRole === 'support' && ['organizations', 'audit', 'support'].includes(area)) ||
  ...
  const menuItems = [
    ...
    ...(can('plans')
      ...
      : []),
    ...(can('analytics') ? [{ key: 'analytics', icon: <BarChartOutlined />, label: 'Analytics' }] : []),
    ...(can('audit')
      ? [
          {
            key: 'audit',
  ...
  const routeFor = (key: string) =>
    key === 'dashboard'
      ...
      : key === 'organizations'
        ...
        : `/platform/${key}`
  const restricted = <Card>Your platform role does not have access to this area.</Card>
  return (
    <Layout className="app-shell platform-shell min-h-screen">
      ...
      >
        <div className="px-6 py-7">
          <Typography.Text className="brand-wordmark font-bold tracking-[.16em]">KRONIQ</Typography.Text>
      ...
      <Layout className="platform-content-shell" style={{ marginLeft: 236, width: 'calc(100% - 236px)' }}>
        <Header
          ...
          <div className="mx-auto flex min-h-[44px] max-w-7xl items-center justify-between gap-3">
            ...
            <div className="flex shrink-0 items-center gap-2">
              <div className="flex items-center rounded-full border border-slate-200 bg-white p-0.5 shadow-sm">
                <Avatar size={30} icon={<UserOutlined />} className="!bg-[#0B1121] !text-white" />
          ...
          <Content className="mx-auto w-full max-w-7xl px-4 py-6 md:px-8">
            <Routes>
              ...
              <Route
                ...
                element={can('organizations') ? <OrganizationList {...pageProps} /> : restricted}
              />
              <Route
                path="/platform/organisations/:organizationId"
                element={can('organizations') ? <OrganizationDetail {...pageProps} /> : restricted}
              />
              <Route
                path="/platform/plans"
                element={can('plans') ? <PlansPage plans={plans} /> : restricted}
              />
              <Route
                path="/platform/analytics"
                element={can('analytics') ? <PlatformAnalyticsPage /> : restricted}
              />
              <Route path="/platform/audit" element={can('audit') ? <PlatformAuditLogPage /> : restricted} />
              <Route
                path="/platform/support"
                element={can('support') ? <PlatformSupportPage organizations={organizations} /> : restricted}
              />
              <Route path="/platform/status" element={can('status') ? <PlatformStatusPage /> : restricted} />
              <Route
                path="/platform/operations"
                element={can('operations') ? <PlatformOperationsPage /> : restricted}
              />
              <Route path="/platform/team" element={<PlatformTeamPage currentRole={platformRole} />} />
            </Routes>
          </Content>
        </div>
