...
export function PlatformTeamPage({ currentRole }: { currentRole: PlatformRole }) {
  ...
  const addMember = async (values: { email: string; password: string; role: PlatformRole }) => {
    ...
    setSaving(false)
    if (error || data?.error) { api.error(data?.error ?? error.message); return }
    api.success('Platform team member added.')
    form.resetFields(); setInviteOpen(false); void load()
  }
  ...
  return <><div className="mb-8 flex flex-wrap items-end justify-between gap-4"><div><Typography.Title level={2} className="!mb-1">Platform team & access</Typography.Title><Typography.Text type="secondary">Small internal RBAC for Kronicle administrators. Only Owners can add members or change roles.</Typography.Text></div>{currentRole === 'owner' && <Button type="primary" icon={<PlusOutlined />} onClick={() => { form.setFieldsValue({ role: 'operator' }); setInviteOpen(true) }}>Add team member</Button>}</div>{holder}<Card title={<><SafetyCertificateOutlined className="mr-2" />Role permissions</>} className="mb-6"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{(Object.entries(descriptions) as [PlatformRole, string][]).map(([role, description]) => <div key={role} className="border p-3"><Tag>{role}</Tag><p className="mb-0 mt-2 text-xs text-slate-500">{description}</p></div>)}</div></Card><Card><Table rowKey="user_id" loading={loading} columns={columns} dataSource={members} pagination={false} scroll={{ x: 760 }} /></Card><Modal title="Add platform team member" open={inviteOpen} onCancel={() => setInviteOpen(false)} onOk={() => void form.submit()} confirmLoading={saving} okText="Add member"><Form form={form} layout="vertical" onFinish={addMember}><Form.Item name="email" label="Work email" rules={[{ required: true, type: 'email' }]}><Input autoFocus /></Form.Item><Form.Item name="password" label="Temporary password" rules={[{ required: true, min: 8, message: 'Use at least 8 characters.' }]}><Input.Password /></Form.Item><Form.Item name="role" label="Platform role" rules={[{ required: true }]}><Select options={(Object.keys(descriptions) as PlatformRole[]).filter((role) => role !== 'owner').map((role) => ({ value: role, label: role[0].toUpperCase() + role.slice(1) }))} /></Form.Item></Form></Modal></>
}
