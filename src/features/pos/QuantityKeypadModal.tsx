...
type Props = {
  ...
  onClose: () => void
  onConfirm: (quantity: number, agreedPrice: number, priceOverrideReason?: string, pack?: ProductPackage) => void
}
...
export function QuantityKeypadModal({ product, maxQuantity, open, role, flexiblePricingEnabled, onClose, onConfirm }: Props) {
  ...
  useEffect(() => { if (open && product) { setQuantityValue(''); setPriceValue(String(Math.round(product.price))); setEditingPrice(false); setReason(''); setSelectedPack(undefined) } }, [open, product?.id])
  const quantity = Number(quantityValue)
  const agreedPrice = Number(priceValue)
  const productFlexiblePricingEnabled = !selectedPack && flexiblePricingEnabled && product?.minimumSellingPrice !== undefined
  ...
  const availableQuantity = selectedPack && product ? Math.floor(product.stock / selectedPack.unitsPerPack) : maxQuantity
  const quantityValid = Number.isInteger(quantity) && quantity > 0 && quantity <= availableQuantity
  const priceValid = Number.isFinite(agreedPrice) && agreedPrice >= 0 && (!priceChanged || (productFlexiblePricingEnabled && (role !== 'cashier' || (cashiersCanChange && agreedPrice <= activePrice && !belowFloor))))
  ...
  return <Modal open={open} title="Add to current sale" footer={null} onCancel={onClose} destroyOnClose>
    <div className="mb-4"><Typography.Text strong className="block text-base">{product.name}</Typography.Text><Typography.Text type="secondary">Default {formatNaira(activePrice)} · {selectedPack ? Math.floor(product.stock / selectedPack.unitsPerPack) : maxQuantity} available to add</Typography.Text></div>
    {!!product.packages?.length && <div className="mb-4"><Typography.Text type="secondary" className="mb-2 block text-xs">Sell as</Typography.Text><div className="flex flex-wrap gap-2"><Button size="small" type={!selectedPack ? 'primary' : 'default'} onClick={() => { setSelectedPack(undefined); setPriceValue(String(Math.round(product.price))); setEditingPrice(false) }}>{product.baseUnit ?? 'Unit'}</Button>{product.packages.map((pack) => <Button key={pack.id} size="small" type={selectedPack?.id === pack.id ? 'primary' : 'default'} onClick={() => { setSelectedPack(pack); setPriceValue(String(Math.round(pack.price))); setEditingPrice(false) }}>{pack.name}</Button>)}</div></div>}
    ...
    <Button type="primary" block size="large" className="mt-5 !h-12" disabled={!valid} onClick={() => { onConfirm(quantity, agreedPrice, priceChanged ? reason.trim() || undefined : undefined, selectedPack); onClose() }}>{priceChanged ? `Add at ${formatNaira(agreedPrice)}` : `Add ${quantityValid ? quantity : ''} to cart`}</Button>
  </Modal>
}
