...
export function ProductManagement({ products, role, onSave }: Props) {
  ...
  const rows = useMemo(() => products.filter((product) => `${product.name} ${product.sku}`.toLowerCase().includes(search.toLowerCase())), [products, search])
  ...
  async function uploadImage(file: File) {
    ...
    if (currentImages().length >= MAX_IMAGES) throw new Error(`A product can have a maximum of ${MAX_IMAGES} images.`)
  ...
  return <>
    ...
    <Card title="Product catalogue">
      <Input.Search value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search product or SKU" className="mb-4 max-w-md" allowClear />
      <Table columns={columns} dataSource={rows} rowKey="id" pagination={{ pageSize: 12 }} />
    </Card>
    <Modal title="Edit product" open={Boolean(editing)} onCancel={() => setEditing(undefined)} onOk={() => void form.submit()} confirmLoading={saving} okText="Save changes" destroyOnClose width={780}>
      <Form form={form} layout="vertical" onFinish={save}>
        <Form.Item name="name" label="Product name" rules={[{ required: true }]}><Input size="large" /></Form.Item>
        <Form.Item name="sku" label="SKU / barcode" rules={[{ required: true }]}><Input size="large" /></Form.Item>
        <Form.Item name="description" label="Online description"><Input.TextArea rows={3} /></Form.Item>
        <Form.Item name="imageUrls" hidden><Input /></Form.Item>
        <Form.Item shouldUpdate noStyle>{() => {
          const images = currentImages()
          return <div className="mb-5">
            <div className="mb-2 flex items-center justify-between"><Text strong>Product images</Text><Text type="secondary" className="text-xs">{images.length} / {MAX_IMAGES}</Text></div>
            <div className="flex flex-wrap items-center gap-3">
              {images.map((url, index) => <div key={url} className="relative h-[76px] w-[76px] overflow-hidden border border-slate-200">
                <Image width={76} height={76} className="object-cover" src={url} preview />
                <Button danger type="text" size="small" icon={<DeleteOutlined />} aria-label={`Remove image ${index + 1}`} className="!absolute right-0 top-0 !bg-white" onClick={() => form.setFieldValue('imageUrls', images.filter((_, imageIndex) => imageIndex !== index))} />
              </div>)}
              {images.length < MAX_IMAGES && role !== 'cashier' && <Upload accept="image/*" showUploadList={false} customRequest={({ file, onSuccess, onError }) => {
                void uploadImage(file as File).then(() => onSuccess?.({})).catch((error: Error) => { api.error(error.message || 'Could not upload image.'); onError?.(error) })
              }}>
                <Button icon={<UploadOutlined />} loading={uploading}>Upload image</Button>
              </Upload>}
            </div>
            <Text type="secondary" className="mt-2 block text-xs">Maximum 2 images per product. Each image can be up to 5 MB.</Text>
          </div>
        }}</Form.Item>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <Form.Item name="active" label="Available for sale">
            <Radio.Group optionType="button" buttonStyle="solid" options={[{ label: 'Inactive', value: false }, { label: 'Active', value: true }]} />
          </Form.Item>
        </div>
