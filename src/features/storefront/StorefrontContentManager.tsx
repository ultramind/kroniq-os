...
const labels: Record<Section['section_type'], string> = {
  text: 'Text section',
...
export function StorefrontContentManager() {
  ...
  const load = useCallback(async () => {
    ...
    const [a, b] = await Promise.all([
      supabase
        ...
        .select('id,section_type,title,body,image_url,cta_label,cta_url,position,visible')
        .eq('store_id', profile.store_id)
        .order('position'),
      supabase
        .from('storefront_testimonials')
        .select('id,customer_name,customer_title,quote,rating,photo_url,verified,published,position')
        .eq('store_id', profile.store_id)
        .order('position'),
    ])
    if (a.error || b.error)
      api.error(a.error?.message ?? b.error?.message ?? 'Could not load storefront content.')
    else {
      setSections((a.data ?? []) as Section[])
      setTestimonials((b.data ?? []) as Testimonial[])
    }
  }, [api])
  useEffect(() => {
    void load()
  ...
  const uploadVisual = async (
    file: File,
    folder: 'sections' | 'testimonials',
    setUrl: (url: string) => void,
  ) => {
    if (!supabase || !storeId) throw new Error('Store profile not found.')
    ...
    try {
      ...
      setUrl(supabase.storage.from('storefront-images').getPublicUrl(path).data.publicUrl)
      api.success('Image uploaded. Save to publish it.')
    } finally {
      setUploading(false)
    }
  }
    ...
    <div className="mt-6 space-y-6">
      ...
      >
        <p className="mb-4 text-sm text-slate-500">
          Add up to eight sections. Use Featured products and Testimonials to show approved content
          automatically.
        </p>
        {sections.length ? (
          <List
            ...
            renderItem={(section) => (
              ...
              >
                <List.Item.Meta
                  title={
                    <>
                      {section.title} {!section.visible && <Tag>Hidden</Tag>}
                    </>
                  }
                  description={`${labels[section.section_type]} · Position ${section.position}`}
                />
              </List.Item>
            )}
          />
        ) : (
          <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No custom sections yet." />
        )}
      </Card>
      <Card
      ...
      >
        {testimonials.length ? (
                ...
                actions={[
                  <Button
                    ...
                    onClick={() => {
                      ...
                      testimonialForm.setFieldsValue(item)
                      setTestimonialOpen(true)
                    }}
                  >
                    Edit
          ...
          <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No testimonials yet." />
        )}
      </Card>
      <Modal
      ...
      >
        <Form form={sectionForm} layout="vertical" onFinish={(values) => void saveSection(values)}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Form.Item name="cta_label" label="Button label">
              <Input />
            </Form.Item>
            <Form.Item name="cta_url" label="Button link">
              <Input />
            </Form.Item>
          </div>
          <div className="grid grid-cols-2 gap-3">
      ...
      >
        <Form form={testimonialForm} layout="vertical" onFinish={(values) => void saveTestimonial(values)}>
          ...
          <Form.Item name="customer_title" label="Customer title / location">
            <Input />
          </Form.Item>
          <Form.Item name="quote" label="Testimonial" rules={[{ required: true }]}>
            <Input.TextArea rows={3} />
          </Form.Item>
          <Form.Item name="rating" label="Rating">
            <Rate />
          </Form.Item>
          <Form.Item name="photo_url" label="Customer photo URL">
            <Input />
          </Form.Item>
