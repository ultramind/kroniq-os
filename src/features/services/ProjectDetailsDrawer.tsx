...
export function ProjectDetailsDrawer({
  ...
  const [brand, setBrand] = useState<InvoiceBrand>({ company_name: 'Your business' })
    ...
    try {
      ...
      const [a, b, c, d, e, f, g, h, i] = await Promise.all([
        supabase
          .from('service_jobs')
          .select(
            'id,store_id,title,description,quoted_amount_kobo,project_date,due_at,status,customer:customers(full_name,phone,email,address),stage:service_workflow_stages(id,name),service:service_catalogue(name)',
          )
          .eq('id', projectId)
          .maybeSingle(),
        supabase
          .from('project_payments')
          .select('id,amount_kobo,payment_method,payment_reference,note,received_at')
          .eq('service_job_id', projectId)
        ...
        supabase
          ...
          .order('position'),
        supabase
          .from('service_job_stage_history')
          .select('id,note,created_at,stage:service_workflow_stages(name)')
          .eq('service_job_id', projectId)
        ...
        supabase
          ...
          .select('id,description,amount_kobo,spent_at')
          .eq('service_job_id', projectId)
        ...
        supabase
          ...
          .eq('store_id', profile.store_id)
          .order('full_name'),
        supabase.from('project_assignments').select('profile_id').eq('service_job_id', projectId),
        supabase.rpc('current_store_invoice_brand').maybeSingle(),
      ])
      const error = [a, b, c, d, e, f, g, h, i].find((result) => result.error)?.error
      if (error) {
        api.error(error.message)
        return
      }
      setProject(a.data as unknown as Project)
      setPayments((b.data ?? []) as Payment[])
      setStages((c.data ?? []) as Stage[])
      setHistory((d.data ?? []) as unknown as History[])
      setDocuments((e.data ?? []) as Document[])
      setExpenses((f.data ?? []) as Expense[])
      setStaff((g.data ?? []) as Staff[])
      setAssigned((h.data ?? []).map((x) => x.profile_id))
      setBrand((i.data as InvoiceBrand | null) ?? { company_name: 'Your business' })
    } finally {
      setLoading(false)
    }
  }, [api, projectId])
  ...
  const totalPaid = useMemo(() => payments.reduce((sum, p) => sum + p.amount_kobo, 0), [payments])
  const totalExpense = useMemo(() => expenses.reduce((sum, e) => sum + e.amount_kobo, 0), [expenses])
  const balance = Math.max(0, (project?.quoted_amount_kobo ?? 0) - totalPaid)
  ...
  const recordPayment = async (values: any) => {
    ...
    const { error } = await supabase.from('project_payments').insert({
      ...
      received_at: (values.receivedAt ?? dayjs()).toISOString(),
      recorded_by: userId,
    })
    if (error) {
      api.error(error.message)
    ...
    api.success('Payment recorded.')
    setPaymentOpen(false)
    paymentForm.resetFields()
    await load()
    onChanged()
  }
  const recordExpense = async (values: any) => {
    if (!supabase || !project) return
    const userId = await currentUser()
    const { error } = await supabase.from('project_expenses').insert({
      ...
      amount_kobo: Math.round(values.amount * 100),
      spent_at: (values.spentAt ?? dayjs()).format('YYYY-MM-DD'),
      recorded_by: userId,
    })
    if (error) {
      api.error(error.message)
  ...
  const updateStage = async (values: any) => {
    if (!supabase || !project) return
    const userId = await currentUser()
    const target = stages.find((stage) => stage.id === values.stageId)
    const { error } = await supabase
      .from('service_jobs')
      .update({ current_stage_id: values.stageId, status: target?.is_terminal ? 'completed' : 'open' })
      .eq('id', project.id)
    if (error) {
      api.error(error.message)
      return
    }
    const { error: historyError } = await supabase.from('service_job_stage_history').insert({
      service_job_id: project.id,
  ...
  const saveAssignments = async (ids: string[]) => {
    if (!supabase || !project) return
    const userId = await currentUser()
    const { error: removeError } = await supabase
      .from('project_assignments')
      .delete()
      .eq('service_job_id', project.id)
    if (removeError) {
      api.error(removeError.message)
      return
    }
    if (ids.length) {
      const { error } = await supabase
        .from('project_assignments')
        .insert(ids.map((profile_id) => ({ service_job_id: project.id, profile_id, assigned_by: userId })))
      if (error) {
        api.error(error.message)
        return
      }
  ...
  const download = async (document: Document) => {
    ...
    if (error || !data) {
      api.error(error?.message ?? 'Could not open document.')
      return
    }
  ...
  const printInvoice = () => {
    ...
    const paymentRows = payments.length
      ? payments
          .map(
            (payment) =>
              `<tr><td>${dayjs(payment.received_at).format('D MMM YYYY')}</td><td>${escape(payment.payment_method)}${payment.payment_reference ? ` · ${escape(payment.payment_reference)}` : ''}${payment.note ? `<br/><span class="muted">${escape(payment.note)}</span>` : ''}</td><td class="right">${formatNaira(payment.amount_kobo / 100)}</td></tr>`,
          )
          .join('')
      ...
      extra={
        <Space>
          ...
          <Button
            ...
            icon={<PlusOutlined />}
            onClick={() => {
              paymentForm.setFieldsValue({ method: 'cash', receivedAt: dayjs() })
              setPaymentOpen(true)
            }}
          >
            Record payment
        ...
        <>
          <div className="grid gap-3 sm:grid-cols-3">
            <Statistic
              ...
              value={project.quoted_amount_kobo / 100}
              formatter={(v) => formatNaira(Number(v))}
            />
            <Statistic title="Received" value={totalPaid / 100} formatter={(v) => formatNaira(Number(v))} />
            <Statistic title="Balance due" value={balance / 100} formatter={(v) => formatNaira(Number(v))} />
          </div>
          <Tabs
            className="mt-6"
            items={[
                ...
                children: (
                  <div className="space-y-6">
                    <Descriptions
                      ...
                      column={{ xs: 1, md: 2 }}
                      items={[
                        { key: 'client', label: 'Client', children: project.customer?.full_name ?? '—' },
                        { key: 'phone', label: 'Phone', children: project.customer?.phone ?? '—' },
                        {
                          key: 'service',
                          label: 'Service',
                        ...
                        },
                        {
                          key: 'stage',
                          label: 'Current stage',
                          children: <Tag>{project.stage?.name ?? 'Not set'}</Tag>,
                        },
                        {
                          key: 'status',
                          label: 'Status',
                          children: <Tag color={statusMeta(project).color}>{statusMeta(project).label}</Tag>,
                        },
                        {
                          key: 'project-date',
                          label: 'Contract date',
                        ...
                        },
                        {
                          ...
                          children: project.due_at ? dayjs(project.due_at).format('D MMM YYYY') : 'Not set',
                        },
                        {
                          key: 'description',
                          label: 'Description',
                    ...
                    <div>
                      <div className="mb-3 flex items-center justify-between">
                        <h3 className="m-0 text-base">Assigned staff</h3>
                        <Select
                          mode="multiple"
                          className="min-w-72"
                          value={assigned}
                          onChange={(ids) => void saveAssignments(ids)}
                      ...
                      {assigned.length ? (
                        <Space wrap>
                          {staff
                            .filter((person) => assigned.includes(person.id))
                            .map((person) => (
                              <Tag key={person.id}>{person.full_name}</Tag>
                            ))}
                        </Space>
                      ) : (
              ...
              },
              {
                ...
                children: (
                  <List
                    dataSource={payments}
                    locale={{ emptyText: 'No payment has been recorded.' }}
                    renderItem={(payment) => (
                      <List.Item>
                        <List.Item.Meta
                          title={formatNaira(payment.amount_kobo / 100)}
                          description={`${dayjs(payment.received_at).format('D MMM YYYY, h:mm A')} · ${payment.payment_method}${payment.payment_reference ? ` · ${payment.payment_reference}` : ''}${payment.note ? ` · ${payment.note}` : ''}`}
                        />
                      </List.Item>
                    )}
                  />
              ...
              },
              {
                ...
                children: (
                  <div className="space-y-4">
                    <Button
                      onClick={() => {
                        stageForm.setFieldsValue({ stageId: project.stage?.id })
                        setStageOpen(true)
                      }}
                    >
                      Update stage
                    ...
                    <Steps
                      ...
                      current={Math.max(
                        0,
                        stages.findIndex((stage) => stage.id === project.stage?.id),
                      )}
                      items={stages.map((stage) => ({ title: stage.name }))}
                    />
                    <List
                      dataSource={history}
                      locale={{ emptyText: 'No stage updates yet.' }}
                      renderItem={(item) => (
                        <List.Item>
                          <List.Item.Meta
                  ...
                  <>
                    <div className="mb-4 flex items-center justify-between">
                      ...
                      <Button
                        icon={<PlusOutlined />}
                        onClick={() => {
                          expenseForm.setFieldsValue({ spentAt: dayjs() })
                          setExpenseOpen(true)
                        }}
                      >
                        Add expense
                      ...
                      renderItem={(expense) => (
                        <List.Item>
                          <List.Item.Meta
                            ...
                            description={dayjs(expense.spent_at).format('D MMM YYYY')}
                          />
                          <span>{formatNaira(expense.amount_kobo / 100)}</span>
                        </List.Item>
                      )}
                    />
                  </>
                ...
                children: (
                  <List
                    ...
                    renderItem={(document) => (
                      ...
                      >
                        <List.Item.Meta
                          avatar={<FilePdfOutlined className="text-xl" />}
                          title={document.file_name}
                          description={`Uploaded ${dayjs(document.created_at).format('D MMM YYYY')}`}
                        />
                      </List.Item>
                    )}
                  />
          ...
          >
            <Form form={paymentForm} layout="vertical" onFinish={(values) => void recordPayment(values)}>
              <Form.Item name="amount" label="Amount received (₦)" rules={[{ required: true }]}>
                <InputNumber min={1} className="w-full" autoFocus />
              </Form.Item>
              <div className="grid grid-cols-2 gap-3">
                <Form.Item name="receivedAt" label="Payment date" rules={[{ required: true }]}>
                  <DatePicker className="w-full" />
                </Form.Item>
                <Form.Item name="method" label="Payment method" rules={[{ required: true }]}>
                  <Select
                    options={['cash', 'transfer', 'card', 'credit'].map((value) => ({ value, label: value }))}
                  />
                </Form.Item>
              </div>
          ...
          >
            <Form form={expenseForm} layout="vertical" onFinish={(values) => void recordExpense(values)}>
              <Form.Item name="description" label="Expense description" rules={[{ required: true }]}>
                <Input autoFocus />
              </Form.Item>
              <div className="grid grid-cols-2 gap-3">
          ...
          >
            <Form form={stageForm} layout="vertical" onFinish={(values) => void updateStage(values)}>
              <Form.Item name="stageId" label="New stage" rules={[{ required: true }]}>
                <Select options={stages.map((stage) => ({ value: stage.id, label: stage.name }))} />
              </Form.Item>
              <Form.Item name="note" label="Client update / internal note">
                <Input.TextArea rows={3} placeholder="Optional progress update" />
              </Form.Item>
            </Form>
          </Modal>
        </>
