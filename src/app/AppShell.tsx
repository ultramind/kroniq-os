...
export function AppShell({ role, pendingSync, syncError, onRetrySync, onReloadCatalogue, children }: Props) {
  ...
  const activeKey = hash === '#audit' ? 'audit'
    ...
    : 'checkout'
  const title = activeKey === 'dashboard' ? 'Dashboard'
    : activeKey === 'audit' ? 'Audit log'
    : activeKey === 'staff' ? 'Staff management'
    : activeKey === 'shifts' ? 'Cash shifts'
    : activeKey === 'reports' ? 'Profit reports'
    : activeKey === 'credits' ? 'Customer credit'
    : activeKey === 'expenses' ? 'Expenses'
    : activeKey === 'products' ? 'Products'
    : activeKey === 'deliveries' ? 'Supplier deliveries'
    : activeKey === 'warehouses' ? 'Warehouses'
    : activeKey === 'sales' ? 'Sales'
    : activeKey === 'inventory' ? 'Inventory'
    : activeKey === 'services' ? 'Service jobs'
    : activeKey === 'settings' ? 'Settings'
    : 'Checkout'
  const menuItems = [
    { key: 'dashboard', icon: <PieChartOutlined />, label: 'Dashboard' },
    ...(retailEnabled ? [{ key: 'pos', icon: <ShoppingCartOutlined />, label: 'Point of sale', children: [
      ...
      ...(role !== 'cashier' ? [{ key: 'credits', icon: <WalletOutlined />, label: 'Customer credit' }] : []),
    ] }] : []),
    ...(servicesEnabled ? [{ key: 'services', icon: <ToolOutlined />, label: 'Services' }] : []),
    ...(retailEnabled && role !== 'cashier'
      ? [
          { key: 'catalogue', icon: <AppstoreOutlined />, label: 'Catalogue & stock', children: [
          ...
          { key: 'finance', icon: <MoneyCollectOutlined />, label: 'Finance', children: [
            { key: 'expenses', icon: <MoneyCollectOutlined />, label: 'Expenses' },
            ...(role === 'admin' ? [{ key: 'reports', icon: <FundOutlined />, label: 'Profit reports' }] : []),
          ] },
        ]
      : retailEnabled ? [{ key: 'stock', icon: <InboxOutlined />, label: 'Stock', children: [{ key: 'inventory', icon: <InboxOutlined />, label: 'Inventory' }] }] : []),
    ...(!retailEnabled && servicesEnabled && role !== 'cashier' ? [{ key: 'finance', icon: <MoneyCollectOutlined />, label: 'Finance', children: [
      { key: 'expenses', icon: <MoneyCollectOutlined />, label: 'Expenses' },
    ] }] : []),
    ...(role === 'admin'
      ? [
  ...
  const activeGroup = ['checkout', 'shifts', 'sales', 'credits'].includes(activeKey) ? 'pos'
    : ['products', 'inventory', 'deliveries', 'warehouses'].includes(activeKey) ? (role === 'cashier' ? 'stock' : 'catalogue')
    : ['expenses', 'reports'].includes(activeKey) ? 'finance'
    : ['staff', 'audit', 'settings'].includes(activeKey) ? 'admin'
    : undefined
    ...
    <Layout className="app-shell min-h-screen">
      ...
      >
        <div className="px-5 py-7">
          <div className="flex min-w-0 items-center gap-3">{tenantBrand.logoUrl ? <img src={tenantBrand.logoUrl} alt="" className="h-8 w-8 shrink-0 object-contain" /> : <span className="grid h-8 w-8 shrink-0 place-items-center bg-[#0B1121] text-xs font-bold text-white">{tenantBrand.name.slice(0, 1).toUpperCase()}</span>}<Text className="brand-wordmark truncate font-bold">{tenantBrand.name}</Text></div>
      ...
      {!collapsed && <button type="button" className="mobile-menu-dismiss md:hidden" aria-label="Close navigation menu" onClick={() => setCollapsed(true)} />}
      <Layout
      ...
      >
        ...
        >
          <div className="mx-auto flex min-h-[44px] max-w-7xl flex-nowrap items-center justify-between gap-3">
            <div className="min-w-0"><h1 className="mb-0 truncate text-xl font-semibold tracking-tight text-slate-900">{title}</h1></div>
            <div className="flex shrink-0 items-center gap-2">
              <Tag color={syncError ? 'error' : supabaseConfigured ? 'green' : 'gold'} className="!m-0 flex !h-9 items-center rounded-full !border-0 !px-2.5 !text-xs !font-medium shadow-sm">
                <CloudSyncOutlined /> <span className="ml-1 hidden md:inline">{syncError ? 'Sync needs attention' : supabaseConfigured ? 'Offline-first' : 'Demo mode'} · </span>{pendingSync} pending
              </Tag>
              ...
              <Tooltip title={mode === 'dark' ? 'Use light theme' : 'Use dark theme'}>
                <Button aria-label="Toggle colour theme" className="theme-toggle !flex !h-9 !w-9 !items-center !justify-center !rounded-full" icon={mode === 'dark' ? <SunOutlined /> : <MoonOutlined />} onClick={toggleTheme} />
              </Tooltip>
              <Dropdown
                trigger={['click']}
                placement="bottomRight"
                dropdownRender={() => <div className="mobile-account-dropdown"><div className="border-b border-slate-100 px-3 py-2.5"><p className="mb-0 max-w-48 truncate text-sm font-semibold">{userName}</p><p className="mb-0 mt-0.5 text-xs capitalize text-slate-500">{role}</p></div>{supabaseConfigured && <Button danger type="text" block className="!mt-1 !flex !h-10 !items-center !justify-start !px-3" icon={<LogoutOutlined />} onClick={() => void supabase?.auth.signOut()}>Sign out</Button>}</div>}
              >
                <Button aria-label="Account menu" className="!flex !h-9 !w-9 !items-center !justify-center !rounded-full md:!hidden" icon={<MoreOutlined />} />
              </Dropdown>
              {supabaseConfigured && <Tooltip title="Sign out"><Button aria-label="Sign out" className="hidden !h-9 !w-9 !items-center !justify-center !rounded-full !border-slate-200 !text-slate-600 hover:!border-red-200 hover:!text-red-600 md:!flex" icon={<LogoutOutlined />} onClick={() => void supabase?.auth.signOut()} /></Tooltip>}
            </div>
          </div>
        </Header>
        <div className="pt-24 md:pt-20">
          {syncError && <div className="flex flex-wrap items-center gap-3 bg-red-50 px-4 py-2 text-sm text-red-700 md:px-8">
            <span>Sync needs attention: {syncError}</span>
            {onRetrySync && <Button size="small" type="primary" onClick={onRetrySync}>Retry sync</Button>}
            {onReloadCatalogue && <Button size="small" onClick={onReloadCatalogue}>Reload catalogue</Button>}
          </div>}
      ...
      <nav className="mobile-bottom-nav md:hidden" aria-label="Mobile navigation">{mobileItems.map((item) => {
        const selected = item.key === 'more' ? !collapsed : item.key === 'cart' ? pathname === '/checkout' && new URLSearchParams(search).has('cart') : activeKey === item.key
        return <button key={item.key} type="button" className={selected ? 'is-active' : ''} onClick={() => { if (item.key === 'more') { setCollapsed(false); return } navigate(item.key === 'dashboard' ? '/' : item.key === 'cart' ? '/checkout?cart=1' : `/${item.key}`); setCollapsed(true) }}><span className="mobile-bottom-nav-icon">{item.badge ? <Badge count={item.badge} size="small">{item.icon}</Badge> : item.icon}</span><span>{item.label}</span></button>
      })}</nav>
    </Layout>
  )
}
