...
export function ReportsPage({ sales, role }: { sales: Sale[]; role: Role }) {
  ...
  const filtered = useMemo(() => {
    ...
    return sales.filter(
      (sale) =>
        sale.createdAt.slice(0, selected.length) === selected &&
        (paymentMethod === 'all' || sale.paymentMethod === paymentMethod),
    )
  }, [paymentMethod, period, sales, selectedDate])
  const costBySale = useMemo(
    () =>
      saleItems.reduce<Record<string, number>>(
        (totals, item) => ({
          ...totals,
          [item.saleId]:
            (totals[item.saleId] ?? 0) +
            (item.costPrice ?? 0) * (item.quantity - (item.returnedQuantity ?? 0)),
        }),
        {},
      ),
    [saleItems],
  )
  const quantityBySale = useMemo(
    () =>
      saleItems.reduce<Record<string, number>>(
        (totals, item) => ({
          ...totals,
  ...
  const returnedBySale = useMemo(
    () =>
      returns.reduce<Record<string, number>>(
        (totals, activity) => ({
          ...totals,
          [activity.saleId]: (totals[activity.saleId] ?? 0) + activity.total,
        }),
        {},
      ),
    [returns],
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
        ...
        <Card title={`Sales in selected ${period}`} className="min-w-0">
          <Table
            rowKey="id"
            size="small"
            scroll={{ x: 940 }}
            pagination={{ pageSize: 12 }}
            dataSource={filtered.slice().sort((a, b) => b.createdAt.localeCompare(a.createdAt))}
            columns={[
              {
                title: 'Date',
                dataIndex: 'createdAt',
              ...
              },
              {
                title: 'Qty',
                key: 'quantity',
                render: (_: unknown, sale: Sale) => quantityBySale[sale.id] ?? 0,
              },
              {
                title: 'Sales total',
                dataIndex: 'total',
              ...
              },
              {
                ...
                key: 'cost',
                render: (_: unknown, sale: Sale) => formatNaira(costBySale[sale.id] ?? 0),
              },
              {
                title: 'Profit',
                key: 'profit',
                render: (_: unknown, sale: Sale) => {
                  const profit =
                    sale.status === 'returned'
                      ? 0
                      : sale.total - (returnedBySale[sale.id] ?? 0) - (costBySale[sale.id] ?? 0)
                  return (
