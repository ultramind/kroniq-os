  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="mb-1 text-xl font-semibold text-slate-900">Product management</h2>
          <p className="mb-0 text-sm text-slate-500">
            Manage catalogue details, pricing, availability, and stock alerts.
          </p>
        </div>
        {role !== 'cashier' && (
          <Button type="primary" icon={<PlusOutlined />} size="large" onClick={onAddProduct}>
            Add product
          </Button>
        )}
      </div>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Card>
          <Statistic title="Active products" value={active.length} />
        </Card>
        <Card>
          <Statistic
            title="Low-stock products"
            value={lowStock}
            valueStyle={{ color: lowStock ? '#d46b08' : undefined }}
          />
        </Card>
        <Card>
          <Statistic
      ...
      <ProductManagement products={products} role={role} onSave={onSaveProduct} />
    </div>
  )
}
