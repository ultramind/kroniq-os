...
type Storefront = { store_name: string; slug: string; template_key?: 'classic_retail' | 'modern_catalogue' | 'premium_brand'; headline?: string; description?: string; phone?: string; whatsapp?: string; address?: string; primary_color?: string; hero_image_urls?: string[]; logo_url?: string; vision?: string; mission?: string; trust_stats?: Array<{ value: string; label: string }> }
...
export function StorefrontPage() {
  ...
  return <main className={`storefront storefront--${store.template_key ?? 'classic_retail'} flex min-h-screen flex-col bg-white text-[#0B1121]`}>
...
function HomePage({ store, products, sections, testimonials, baseUrl, actions, primaryColor, template }: { store: Storefront; products: Product[]; sections: Section[]; testimonials: Testimonial[]; baseUrl: string; actions: React.ReactNode; primaryColor: string; template: NonNullable<Storefront['template_key']> }) {
  ...
  return <>
    <StorefrontHero store={store} products={products} actions={actions} primaryColor={primaryColor} template={template} />
...
function StorefrontHero({ store, products, actions, primaryColor, template }: { store: Storefront; products: Product[]; actions: React.ReactNode; primaryColor: string; template: NonNullable<Storefront['template_key']> }) {
  const title = store.headline || 'Everything you need, from a store you can trust.'
  const description = store.description || 'Browse our latest products and place your order directly with our team.'
  const image = store.hero_image_urls?.[0]
  if (template === 'modern_catalogue') return <section className="border-b border-slate-200 bg-slate-50"><div className="mx-auto max-w-7xl px-5 py-12 sm:py-16"><p className="text-xs font-semibold uppercase tracking-[.2em] text-slate-500">{store.store_name} · Online catalogue</p><div className="mt-5 grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]"><div><h1 className="max-w-3xl text-4xl font-semibold tracking-tight text-[#0B1121] sm:text-6xl">{title}</h1><p className="mt-5 max-w-2xl text-lg leading-8 text-slate-600">{description}</p><div className="mt-8">{actions}</div></div><div className="border border-slate-200 bg-white p-6"><p className="text-sm text-slate-500">Live catalogue</p><p className="mt-2 text-5xl font-semibold text-[#0B1121]">{products.length}</p><p className="mt-3 text-sm leading-6 text-slate-500">Products ready to browse, with current prices and availability.</p></div></div></div></section>
  if (template === 'premium_brand') return <section className="relative isolate min-h-[520px] overflow-hidden bg-[#0B1121] text-white">{image && <img src={image} alt="" className="absolute inset-0 -z-20 h-full w-full object-cover opacity-45" />}<div className="absolute inset-0 -z-10 bg-gradient-to-b from-[#0B1121]/45 via-[#0B1121]/65 to-[#0B1121]" /><div className="mx-auto flex min-h-[520px] max-w-5xl flex-col items-center justify-center px-5 py-20 text-center"><p className="text-xs font-semibold uppercase tracking-[.26em] text-slate-300">{store.store_name}</p><h1 className="mt-6 max-w-4xl font-serif text-5xl leading-[1.02] sm:text-7xl">{title}</h1><p className="mt-7 max-w-2xl text-lg leading-8 text-slate-200">{description}</p><div className="mt-10">{actions}</div><div className="mt-12 border border-white/30 px-5 py-3 text-sm text-slate-100">A thoughtful collection of {products.length} available products</div></div></section>
  return <section style={{ backgroundColor: primaryColor }} className="overflow-hidden text-white"><div className="mx-auto grid min-h-[480px] max-w-7xl lg:grid-cols-2"><div className="flex flex-col justify-center px-5 py-16 sm:px-10 lg:px-14"><p className="mb-4 text-xs font-semibold uppercase tracking-[0.2em] text-slate-300">Welcome to {store.store_name}</p><h1 className="max-w-2xl text-4xl font-semibold leading-tight sm:text-5xl">{title}</h1><p className="mt-5 max-w-xl text-base leading-7 text-slate-200">{description}</p><div className="mt-8">{actions}</div></div><HeroImageCarousel images={store.hero_image_urls ?? []} storeName={store.store_name} /></div></section>
}
...
function HeroImageCarousel({ images, storeName }: { images: string[]; storeName: string }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setActiveImage((current) => (current + 1) % visibleImages.length), 5000)
    return () => window.clearInterval(timer)
  }, [visibleImages.length])
  return <div className="hero-image-carousel relative min-h-[330px] overflow-hidden bg-slate-900 lg:min-h-full">{visibleImages.length ? visibleImages.map((image, index) => <img key={image} src={image} alt={`${storeName} storefront`} className={`absolute inset-0 h-full w-full object-cover transition-all duration-700 ${index === activeImage ? 'scale-100 opacity-100' : 'scale-105 opacity-0'}`} />) : <div className="grid h-full place-items-center bg-black/15 text-sm text-white/70">Storefront image</div>}{visibleImages.length > 1 && <div className="absolute bottom-5 left-1/2 flex -translate-x-1/2 gap-2">{visibleImages.map((image, index) => <button key={image} type="button" aria-label={`Show hero image ${index + 1}`} onClick={() => setActiveImage(index)} className={`h-1.5 w-7 transition ${index === activeImage ? 'bg-white' : 'bg-white/45'}`} />)}</div>}</div>
}
...
function ProductGrid({ products, baseUrl, empty }: { products: Product[]; baseUrl: string; empty: string }) {
  return products.length ? <div className="grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-3 xl:grid-cols-4">{products.map((product) => <ProductCard key={product.id} product={product} href={`${baseUrl}/products/${product.id}`} />)}</div> : <Empty description={empty} />
}
...
function ProductCard({ product, href }: { product: Product; href: string }) {
  ...
  return <Link to={href} className="storefront-product-card group block overflow-hidden border border-slate-200 bg-white transition duration-200 hover:-translate-y-1 hover:border-slate-300 hover:shadow-[0_14px_34px_rgb(11_17_33_/_10%)]"><div className="relative aspect-square overflow-hidden bg-slate-100">{image ? <img className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.06]" src={image} alt={product.name} /> : <div className="grid h-full place-items-center text-center text-xs text-slate-400">No product image</div>}<span className="absolute bottom-2 left-2 max-w-[calc(100%-16px)] truncate bg-white/95 px-2 py-1 text-[10px] font-semibold text-slate-700 shadow-sm backdrop-blur">{product.category || 'Products'}</span></div><div className="p-3 sm:p-4"><h3 className="line-clamp-2 min-h-10 text-sm font-semibold leading-5 text-[#0B1121] sm:text-base">{product.name}</h3><div className="mt-3 flex items-center justify-between gap-2"><p className="m-0 truncate text-sm font-bold text-[#0B1121] sm:text-base">{formatNaira(product.price_kobo / 100)}</p><span className="grid h-7 w-7 shrink-0 place-items-center border border-slate-200 text-xs text-slate-500 transition group-hover:border-[#0B1121] group-hover:bg-[#0B1121] group-hover:text-white"><RightOutlined /></span></div></div></Link>
}
