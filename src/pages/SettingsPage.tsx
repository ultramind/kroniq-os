...
export function SettingsPage({ role }: { role: Role }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!profile) return
      const { data: store } = await supabase.from('stores').select('organization_id,flexible_pricing_enabled').eq('id', profile.store_id).maybeSingle()
      if (!store) return
      if (store.flexible_pricing_enabled !== undefined) form.setFieldValue('flexiblePricingEnabled', store.flexible_pricing_enabled)
  ...
  const saveSettings = async (values: Values) => {
    ...
    const [organizationResult, checkoutResult] = await Promise.all([
      ...
      supabase.rpc('update_current_store_checkout_settings', { p_flexible_pricing_enabled: values.flexiblePricingEnabled }),
    ])
