...
export interface Product { id: string; name: string; sku: string; price: number; costPrice: number; stock: number; category: string; lowStockThreshold?: number; active?: boolean; description?: string; imageUrl?: string; imageUrls?: string[]; onlinePublished?: boolean; featured?: boolean }
...
export interface SaleItem { id: string; saleId: string; productId: string; productName: string; quantity: number; unitPrice: number; costPrice?: number; returnedQuantity?: number }
